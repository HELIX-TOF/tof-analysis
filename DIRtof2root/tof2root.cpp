...
int readevent(FILE *file, struct tof_evt *p) {
  ...
  while (1) {
    ...
    if ((p->hw_nbytes=*((uint16_t *) (buf+2))) != 2*(16*NSAMPLES+8) ) {
      fprintf(stderr,"bad hw length %d, expected %d, dropping event\n",p->hw_nbytes,2*(16*NSAMPLES+8));
      dropit=1;
    }
    ...
    p->data.ctime_tof = *((uint16_t *) (buf+14));

    for(i=0;i<16;i++)
      for(j=0;j<NSAMPLES;j++) 
		  {
	      x = *((uint16_t *) (buf+16+2*(NSAMPLES*i+j)));
	      p->data.adc[i][j] = 16384 - (x & 0x3fff);   // restore correct polarity here (maybe we move this to firmware????)
	      p->data.flags[i][j] = (x & 0xc000)>>14;
      }

    p->data.t1 = *((uint16_t *) (buf+16+2*(NSAMPLES*16)));
    p->data.t2 = *((uint16_t *) (buf+16+2*(NSAMPLES*16)+2));
    p->data.length = *((uint16_t *) (buf+16+2*(NSAMPLES*16)+4));
    if ((p->data.endmarker = *((uint16_t *) (buf+16+2*(NSAMPLES*16)+6))) != 0xf007) {
